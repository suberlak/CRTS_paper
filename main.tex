...
of CRTS light curves. This short timescale implies much stronger variability on monthly time scales than observed in SDSS 
...
The quasar catalog\footnote{\url{http://www.astro.washington.edu/users/ivezic/cmacleod/qso_dr7/Southern.html}} includes spectroscopically confirmed quasars from the SDSS Data Release 7 \citep{abazajian2009}, based on the SDSS Quasar Catalog IV \citep{schneider2008}, and was compiled by \cite{macleod2011}. The standard stars
...
In this study we used a sample of 7,932 spectroscopically confirmed S82 quasars from  the Data Release 2, based 
...
mean interval between two succesive observing nights of 209 {\bf XXX this cannot be correct!} 
days (see Fig.~1). We also use CRTS light curves for 52,133 randomly chosen 10\% subsample of the S82 
...
The bottom-left panel shows the  mean day-averaged light curve error, $\langle \sigma_{j} \rangle$ (see eq.~\ref{eq:2}). 
We use only quasars with light curve averaged error smaller than 0.3, leaving 7,108 quasars in the sample.
The bottom-right panel shows the average time difference (per quasar) between day-averaged epochs.}
\label{fig:1}
\end{figure}
...
{\bf XXX this whole section makes no sense - the starting catalog from MacLeod already includes
all the relevant SDSS data! I propose to remove this entire section} 
...
for individual photometric data points computed by the CRTS photometric pipeline. 
...
To find magnitude difference between different observing nights, we first require that the raw light curve 
must have more than 10 epochs. This step reduces the sample sizes from the initial 52,131 stars and 7,932 
quasars to 49,385 stars and 7,707 quasars. We thus also remove those light curves with less than 10 
observing nights, leaving 48,250 stars and 7,601. {\bf XXX the last two statements sound identical - what's
the difference between ``10 epochs'' and ``10 nights''?} In addition, we require that the light curve-average 
...
and $e$ for $e_{j,k}$) by binning ($\Delta t$, $\Delta m$, $e$) data along $\Delta t$ axis.  With a median number of data points per light curve of 
...
\begin{figure}
\includegraphics[width=1.0\columnwidth, center]{{Fig_2_18.5-19_panels}.png}
\vskip -0.15in
\caption{The four panels show various statistics computed for subsamples of 333 CRTS quasars (black points), 
1400 "blue" stars (blue points), and 2087 "red" stars (red points), with SDSS $r$ magnitudes in the range 18.5-19.
{\bf XXX according to table 1, these counts correspond to 18-18.5, not 18.5-19!.}  Red and blue stars have  
SDSS colors $1 < \mathrm{g-i} < 3$ and  $-1 < \mathrm{g-i} < 1$, respectively. All pairwise CRTS brightness 
differences are binned in 200 linearly spaced bins of time difference $\Delta t$. For each bin, we compute,
from top to bottom: the standard deviation $\sigma_{stdev}$, the robust standard deviation estimate based on 
the interquartile range $\sigma_{G}$, the structure function SF, and the mean value of $\Delta m$ per bin $\mu$. 
Both $\mu$ and SF are found from the 2D maximum of the log-likelihood $L_{p}$ on the $[\mu,\rm{SF}]$ grid
(see eq.~\ref{eq:logP}).  The yellow dashed line in the third panel traces the fiducial Damped Random Walk model
(see eq.~\ref{eq:DRWSF}).}
...
The bottem two panels in Fig.~\ref{fig:2} show SF and $\mu$ as a function of $\Delta t$ for quasars, blue 
...
all three subsamples. In case of quasars, the observed SF$\sim$0.1$^m$ for 10 d $<\Delta t<$100 d
...
methods (SF vs. SWV, respectively): here we fully reproduce this discrepancy using SF method and 
CRTS data. 

Fig.~\ref{fig:2} also indicates a plausible solution to this puzzle: the observed SF for both blue and red
stars in the range 10$^d<\Delta t<$100$^d$ is {\it unexpectedly} range: the values are in the range 
...
\begin{figure}
%\centerfloat
\includegraphics[width=1.00\columnwidth, center]{Fig_3_histogram_panels.png}
\vskip -0.15in
...
for blue stars will be unity, and that their SF will tend to 0. For quasars, we expect somewhat smaller SF
at large $\Delta t$ and much smaller SF at small $\Delta t$, compared to SF values shown in the third 
panel in Fig.~\ref{fig:2}.
...
\begin{figure}
\vskip -0.15in
\includegraphics[width=1.0\columnwidth, center]{Fig_4_SF_QSO_starsB_r_cut.png}
...
\label{fig:5}
\vskip -0.15in
\includegraphics[width=1.0\columnwidth, center]{Fig_5_SF_QSO_starsB_r_cut_rest.png}
...
Recently PTF (Palomar Transient Factory) Data Release 3 light curves\footnote{\url{http://www.ptf.caltech.edu/page/lcdb}}
...
\centering
\caption{Count of stars and quasars, selected by their SDSS $r$ magnitudes and $g-i$ colors.}
\caption{Analoguous to ~\ref{tab:object_count}, except that here the counts of stars and quasars with PTF
...
\caption{Analogous to Fig.~\ref{fig:2}, but here the statistics for subsamples of 377 quasars (black points), 548 ``blue'' stars (blue points), 
and 913 ``red'' stars (red points), with adequate PTF light curve data are shown. Note that the mean magnitude difference ($\mu$, 
the bottom panel) does not stay as close to 0 as for CRTS data -- a deviation around $\log_{10} \Delta t \approx 2.7$ might indicate 
some issues with photometric zeropoint calibration (at the level of 0.02-0.03$^m$).}
\label{fig:2PTF} 
\end{figure}
...
resulting corrections are thus too small. For small $\Delta t$, where quasar SF is intrinsically 
