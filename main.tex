...
The CRTS survey (Drake+2009) telescopes use 4kx4k CCDs, and observations are made in an unfiltered white light (Djorgovski+2011).  As part of the CRTS pipeline this unfiltered photometry is calibrated to the V-band zero point. Our CRTS Quasar sample consists of 7754 Quasars from the equatorial region of the sky ($22h 24m < R.A. < 04h 08m$ and $| Dec | < 1.27 deg$)  known as the Stripe 82 (Branimir Sesar, priv comm.). We select lightcurves with more than 10 measurements, which leaves 7707 objects. Within that sample , $96 \% $ observations span the time of 7-9 years. The light curve and sample-averaged mean and median error is 0.22. The mean  brightness is 19.50, and the mean number of individual observations per light curve  is 209. $91.2\%$ of  quasars were observed between 1 to 4 times per night.  (See Fig.\ref{fig:CRTS_QSO_stats})
...
We use CRTS standard stars as  reference objects that exhibit no intrinsic variability (thus SF=0).  To ensure a uniform sample we impose the following selection criteriaS. We select CRTS quasars and stars  based on the SDSS  r-band photometry $17< m_{SDSS,r} < 20$, and the CRTS lightcurve-averaged error  $0.05 < \langle CRTS_{err} \rangle < 0.3$. To assess the influence of colors on variability,  we divide the comparison stars into two color bins: "red" with   $1 < g-i < 3$ and "blue" $-1 < g-i < 1$,  using SDSS $g$ and $i$ filters .
...
As seen on Fig.~\ref{fig:panel_plots},  the quasar variability on short timescales departs from the DRW model, and non-variable stars show a non-zero variability on a level larger than allowed by photometric errors.  To analyze the error properties of the short timescale points we select $\log{\tau} < 1.7$ points for both  quasars and stars, which includes $721 283$ time-lag points based on quasars, and $314 248$ time-lag  stellar points. To disentangle the magnitude sensitivity, we further split the sample into  three  magnitude bins : $17-18$,  $18-18.5$ and $18.5-19$ $^{mag}$. 

Assuming that each $\delta m$ data point originates from a homoscedastic Gaussian distribution, with a standard deviation $\sigma_{com}$ (that includes all intrinsic variability $\sigma$ and error-related $f_{c}*e_{i}$):
